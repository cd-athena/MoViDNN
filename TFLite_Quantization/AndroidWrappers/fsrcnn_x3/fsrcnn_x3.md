...
```
import com.example.tflitesr.fsrcnn_x3.FSRCNN_X3;

// 1. Initialize the Model
FSRCNN_X3 model = null;

try {
    model = FSRCNN_X3.newInstance(context);  // android.content.Context
} catch (IOException e) {
    e.printStackTrace();
}
...
if (model != null) {
    ...
    // 3. Run the model
    FSRCNN_X3.Outputs outputs = model.process(lrImage);
