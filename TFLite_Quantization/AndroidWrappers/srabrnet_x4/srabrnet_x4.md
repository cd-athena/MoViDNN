...
```
import com.example.tflitesr.srabrnet_x4.SRABRNET_X4;

// 1. Initialize the Model
SRABRNET_X4 model = null;

try {
    model = SRABRNET_X4.newInstance(context);  // android.content.Context
} catch (IOException e) {
    e.printStackTrace();
}
...
if (model != null) {
    ...
    // 3. Run the model
    SRABRNET_X4.Outputs outputs = model.process(lrImage);
